...
{
	my_logger = spdlog::basic_logger_mt("Datarecorder", "logs/datarecorder.txt");
	try {
...
void Datarecorder::onTick(std::shared_ptr<Event>e)
{
	try {
		...
		my_logger->error("onTick()");
	}
}

void Datarecorder::OnBar(const BarData &bar)
{
	try {
...
void Datarecorder::insertTicktoDb(const std::string &dbname,const std::string &symbol, std::shared_ptr<Event_Tick> tick)
{
	try {
		...
		my_logger->error("insertTicktoDb()");
	}
}
