...
{
    QBarCategoryAxis *axisX = qobject_cast<QBarCategoryAxis *>(m_categoriesAxis);
    const int tickCount = axisX->tickCount();

    QVector<qreal> points;
    const QRectF& gridRect = gridGeometry();
    qreal range = max() - min();
    const qreal delta = gridRect.width() / tickCount;
...
QSizeF ChartBarCategoryAxisX::sizeHint(Qt::SizeHint which, const QSizeF &constraint) const
{
    ...
    QStringList tempList = m_categoriesAxis->categories();
    QStringList ticksList;
    for(int i=0;i<tempList.count();++i)
    {
        if((i+1)%tickCount==0)
        {
            ticksList<<tempList[i];
        }
    }
